...
async function getSmartSuggestion(payload) {
  try {
    ...
    // tie-break: ưu tiên % thấp hơn nếu profit bằng nhau
    const bestDisc = discData.series.reduce((best, cur) => {
      if (cur.profit > best.profit) return cur;
      if (cur.profit === best.profit && cur.x < best.x) return cur;
      return best;
    });
...
async function drawDiscountChart() {
  ...
  Plotly.newPlot("discount-chart", traces, {
...
async function downloadReport() {
  ...
  let filename = "BaoCaoGiaKhuyenMai.xlsx";
